...
def build_parser() -> argparse.ArgumentParser:
    ...
    parser.add_argument(
        "--output-base",
        default="outputs/e2e_runs",
        help="Base directory for e2e run folders. Defaults to existing outputs/e2e_runs.",
    )
    ...
    parser.add_argument(
        ...
        help=(
            "Train QA JSON filename under the train co_llm directory, or an "
            "absolute path to an isolated leak-filtered train QA JSON."
        ),
    )
...
def write_train_markdown(path: Path, manifest: dict[str, object]) -> None:
    ...
    lines = [
        ...
        "",
        "```bash",
        f"python3 scripts/e2e/run_gmark_ablation_train_pipeline.py --run-name {manifest.get('run_name', '')}",
        (
...
def run_train_evals(
    *,
    ...
    train_output_root = e2e_root / "phase8_train_dev"
    ...
    for task in task_configs:
        ...
        run(command)
        src_json = train_output_root / "official_eval_reports" / f"{scenario}_official_export_manifest_official_qa_eval_summary.json"
        src_md = train_output_root / "official_eval_reports" / f"{scenario}_official_export_manifest_official_qa_eval_summary.md"
        src_export_manifest = train_output_root / "official_exports" / f"{scenario}_official_export_manifest.json"
...
def main() -> int:
    ...
    e2e_root = output_base / args.run_name
    ...
    print(f"train_file_name: {args.train_file_name}")
